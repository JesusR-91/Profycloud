...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { email, password, firstName, lastName, tutorClass, classes } =
        req.body;
...
router.post("/class/:idClass/edit", async (req, res, next) => {
  try {
    ...
    const editClass = await Class.findByIdAndUpdate(req.params.idClass, {
      name,
      subName,
    });
    ...
    for (let i = 0; i < allProfessors.length; i++) {
      if (
        profArray.includes(allProfessors[i]._id.toString()) === true &&
        allProfessors[i].class.includes(editClass._id) === false
      ) {
        await User.findByIdAndUpdate(allProfessors[i]._id, {
          $push: { class: editClass._id },
        });
      } else if (
        allProfessors[i].class.includes(editClass._id) === true &&
        profArray.includes(allProfessors[i]._id.toString()) === false
      ) {
        await User.findByIdAndUpdate(allProfessors[i]._id, {
          $pull: { class: editClass._id },
        });
      }
    }
...
router.post("/class/create/newClass", async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < professors.length; i++) {
      await User.findByIdAndUpdate(professors[i], {
